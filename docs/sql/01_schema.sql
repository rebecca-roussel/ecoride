...
CREATE TABLE voiture (
  ...
  CONSTRAINT ck_voiture_desactivation CHECK (
    ...
    OR (est_active = false AND date_desactivation IS NOT NULL)
  ),
  ...
  CONSTRAINT fk_voiture_utilisateur FOREIGN KEY (id_utilisateur) REFERENCES utilisateur(id_utilisateur)
);
...
CREATE TABLE covoiturage (
  ...
  -- liste fermée du statut du covoiturage
  CONSTRAINT ck_covoiturage_statut CHECK (statut_covoiturage IN ('PLANIFIE','EN_COURS','TERMINE','ANNULE','INCIDENT')),
  ...
  CONSTRAINT ck_incident_commentaire_obligatoire CHECK (
    ...
    OR (statut_covoiturage = 'INCIDENT' AND length(trim(incident_commentaire)) > 0)
  ),

  id_utilisateur INTEGER NOT NULL,
  id_voiture INTEGER NOT NULL,
  id_chauffeur INTEGER NOT NULL,
  -- La clé étrangère liée au chauffeur (conducteur)
  CONSTRAINT fk_covoiturage_chauffeur FOREIGN KEY (id_chauffeur) REFERENCES utilisateur(id_utilisateur),

  id_passager INTEGER NOT NULL,
  -- La clé étrangère liée au passager
  CONSTRAINT fk_covoiturage_passager FOREIGN KEY (id_passager) REFERENCES utilisateur(id_utilisateur),

  PRIMARY KEY (id_covoiturage),
  CONSTRAINT fk_covoiturage_utilisateur FOREIGN KEY (id_utilisateur) REFERENCES utilisateur(id_utilisateur),
  CONSTRAINT fk_covoiturage_voiture FOREIGN KEY (id_voiture) REFERENCES voiture(id_voiture)
);
...
CREATE TABLE participation (
  ...
  CONSTRAINT fk_participation_utilisateur FOREIGN KEY (id_utilisateur) REFERENCES utilisateur(id_utilisateur),
  CONSTRAINT fk_participation_covoiturage FOREIGN KEY (id_covoiturage) REFERENCES covoiturage(id_covoiturage)
);

CREATE TABLE employe (
  id_utilisateur INTEGER NOT NULL,
  PRIMARY KEY (id_utilisateur),
  CONSTRAINT fk_employe_utilisateur FOREIGN KEY (id_utilisateur) REFERENCES utilisateur(id_utilisateur)
);

CREATE TABLE administrateur (
  id_utilisateur INTEGER NOT NULL,
  PRIMARY KEY (id_utilisateur),
  CONSTRAINT fk_administrateur_utilisateur FOREIGN KEY (id_utilisateur) REFERENCES utilisateur(id_utilisateur)
);
...
CREATE TABLE avis (
  ...
  CONSTRAINT fk_avis_participation FOREIGN KEY (id_participation) REFERENCES participation(id_participation),

  id_employe_moderateur INTEGER NULL,
  -- Identifie l’employé ayant modéré l’avis (si modération effectuée)
  CONSTRAINT fk_avis_employe FOREIGN KEY (id_employe_moderateur) REFERENCES employe(id_utilisateur),
